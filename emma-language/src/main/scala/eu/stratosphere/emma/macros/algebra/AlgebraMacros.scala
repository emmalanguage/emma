...
class AlgebraMacros(val c: Context) {
  ...
  // monad
  def map[T, U](f: c.Expr[T => U])(in: c.Expr[DataSet[T]]): c.Expr[DataSet[U]] = {
    val tree = q"$in.map($f)"
    c.Expr[DataSet[U]](tree)
  }

  def flatMap[T, U](f: c.Expr[T => DataSet[U]])(in: c.Expr[DataSet[U]]): c.Expr[DataSet[U]] = {
    val tree = q"$in.flatMap($f)"
    c.Expr[DataSet[U]](tree)
  }

  def filter[T](p: c.Expr[T => Boolean])(in: c.Expr[DataSet[T]]): c.Expr[DataSet[T]] = {
    val tree = q"$in.withFilter($p)"
    c.Expr[DataSet[T]](tree)
  }

  // cross
  def cross2[T1, T2](in1: c.Expr[DataSet[T1]], in2: c.Expr[DataSet[T1]]) = {
    val tree = q"for (i1 <- $in1; i2 <- $in2) yield (i1, i2)"
    c.Expr[DataSet[(T1, T2)]](tree)
  }

  def cross3[T1, T2, T3](in1: c.Expr[DataSet[T1]], in2: c.Expr[DataSet[T1]], in3: c.Expr[DataSet[T2]]) = {
    val tree = q"for (i1 <- $in1; i2 <- $in2; i3 <- $in3) yield (i1, i2, i3)"
    c.Expr[DataSet[(T1, T2, T3)]](tree)
  }

  def cross4[T1, T2, T3, T4](in1: c.Expr[DataSet[T1]], in2: c.Expr[DataSet[T1]], in3: c.Expr[DataSet[T2]], in4: c.Expr[DataSet[T4]]) = {
    val tree = q"for (i1 <- $in1; i2 <- $in2; i3 <- $in3; i4 <- $in4) yield (i1, i2, i3, i4)"
    c.Expr[DataSet[(T1, T2, T3, T4)]](tree)
  }

  def cross5[T1, T2, T3, T4, T5](in1: c.Expr[DataSet[T1]], in2: c.Expr[DataSet[T2]], in3: c.Expr[DataSet[T3]], in4: c.Expr[DataSet[T4]], in5: c.Expr[DataSet[T5]]) = {
    val tree = q"for (i1 <- $in1; i2 <- $in2; i3 <- $in3; i4 <- $in4; i5 <- $in5) yield (i1, i2, i3, i4, i5)"
    c.Expr[DataSet[(T1, T2, T3, T4, T5)]](tree)
  }

  // join: for (i1 <- in1; i2 <- in2; if k1(i1) == k2(i2)) yield (i1, i2)
  def join[T1: c.WeakTypeTag, T2:  c.WeakTypeTag, K:  c.WeakTypeTag](k1: c.Expr[T1 => K], k2: c.Expr[T2 => K])(in1: c.Expr[DataSet[T1]], in2: c.Expr[DataSet[T2]]): c.Expr[DataSet[(T1, T2)]] = {
  ...
  def groupBy[T, K](k: c.Expr[T => K])(in: c.Expr[DataSet[T]]): c.Expr[DataSet[DataSet[T]]] = {
