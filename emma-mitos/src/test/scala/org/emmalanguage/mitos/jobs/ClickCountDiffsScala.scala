...
import mitos.LabySource
import mitos.operators.CFAwareFileSink
import mitos.operators.ClickLogReader
import mitos.operators.ConditionNode
import mitos.operators.IncMap
import mitos.operators.OuterJoinTupleIntInt
import mitos.operators.SmallerThan
import mitos.operators.Sum
import mitos.operators.SumCombiner
import mitos.partitioners.Always0
import mitos.partitioners.Forward
import mitos.partitioners.IntegerBy0
import mitos.partitioners.RoundRobin
import mitos.partitioners.TupleIntIntBy0
import mitos.util.TupleIntInt
...
object ClickCountDiffsScala {
  ...
  def main(args: Array[String]): scala.Unit = {
    ...
    env.addSource(kickoffSrc).addSink(new DiscardingSink[mitos.util.Unit])
    ...
    val ifCond = new LabyNode[java.lang.Boolean, mitos.util.Unit](
      "ifCond",
      new ConditionNode(Array[Int](2, 3), Array[Int](3)),
      1,
      new Always0[java.lang.Boolean](1),
      booleanSer,
      TypeInformation.of(new TypeHint[ElementOrEvent[mitos.util.Unit]]() {})
    )
    ...
    val exitCond = new LabyNode[java.lang.Boolean, mitos.util.Unit](
      "exitCond",
