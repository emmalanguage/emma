...
import mitos.util.SerializedBuffer
import mitos.BagOperatorOutputCollector
import mitos.CFLConfig
import mitos.ElementOrEvent
import mitos.KickoffSource
import mitos.LabyNode
import mitos.partitioners.Partitioner
import mitos.util.SocketCollector
...
object ScalaOps {
  ...
  def fromNothing[OUT](f: () => OUT ): BagOperator[org.emmalanguage.mitos.util.Nothing,OUT] = {
    new BagOperator[org.emmalanguage.mitos.util.Nothing,OUT]() {
  ...
  def empty[OUT]: BagOperator[org.emmalanguage.mitos.util.Nothing, OUT] = {
    new BagOperator[org.emmalanguage.mitos.util.Nothing,OUT]() {
      override def openOutBag() : Unit = {
        super.openOutBag()
        out.closeBag()
  ...
  : BagOperator[Boolean, org.emmalanguage.mitos.util.Unit] = {
  ...
  def collectToClient[T](env: StreamExecutionEnvironment, in: LabyNode[_, T], bbId: Int) = {
    mitos.util.Util.collect(env.getJavaEnv, in, bbId)
  }
}
...
class ConditionNodeScalaReally(val trueBranchBbIds: Array[Int], val falseBranchBbIds: Array[Int])
  extends SingletonBagOperator[Boolean, mitos.util.Unit] {
...
object LabyStatics {
  ...
  def setKickoffSource(ints: Int*)(implicit env: StreamExecutionEnvironment): Unit = {
    ...
    env.addSource(kickoffSrc)(org.apache.flink.api.scala.createTypeInformation[org.emmalanguage.mitos.util.Unit])
      .addSink(new DiscardingSink[org.emmalanguage.mitos.util.Unit])
  }
  ...
  def executeAndGetCollectedNonBag[T: Meta](env: StreamExecutionEnvironment, socColl: SocketCollector[T]): T = {
    val arrayList: util.ArrayList[T] = mitos.util.Util.executeAndGetCollected(env.getJavaEnv, socColl)
  ...
  def executeWithCatch(env: StreamExecutionEnvironment): Unit = {
    mitos.util.Util.executeWithCatch(env.getJavaEnv)
  }
}