...
import api.Meta
import api.alg.Alg
import mitos.operators.ScalaOps

import org.apache.flink.api.common.typeinfo.TypeInformation

trait mitosCompilerBase extends Compiler {
  ...
  object ElementOrEventAPI extends ClassAPI {
    lazy val sym = api.Sym[mitos.ElementOrEventTypeInfo[Any]].asClass
    override def ops = Set()
  }

  object LabyStatics$ extends ModuleAPI {
    lazy val sym = api.Sym[mitos.operators.LabyStatics.type].asModule
  ...
  object LabyNodeAPI extends ClassAPI {
    lazy val sym = api.Sym[mitos.LabyNode[Any,Any]].asClass
  ...
  implicit def typeInfoForType[T](implicit meta: api.Meta[T]): TypeInformation[T] = {
    ...
    if (memo.contains(tpe)) memo(tpe).asInstanceOf[TypeInformation[T]]
      ...
      s"""
        ...
        |Try calling `Memo.memoizeTypeInfo for this type` explicitly before the `emma.onmitos` quote.
      """.stripMargin.trim
    )
  }
