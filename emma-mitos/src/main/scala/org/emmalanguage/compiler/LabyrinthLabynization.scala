...
import mitos.operators.InputFormatWithInputSplit
import api.Group

import org.apache.flink.core.fs.FileInputSplit
import shapeless.::

trait mitosLabynization extends mitosCompilerBase {

  import API._
  import UniverseImplicits._

  val mitosLabynize: TreeTransform = TreeTransform("mitosLabynize", (tree: u.Tree) => {
    ...
    api.TopDown
      ...
      .traverseWith {
        ...
        els @ core.DefCall(_, elsSym, _, _)
          ...
          val outTpe = getTpe[mitos.util.Unit]
    ...
    // add mitos statics to beginning and end of code
        ...
        val execDCRefDef = exprSymOpt match {
          ...
          case Some(exprSym) =>
            ...
            val ortIsBag = meta(tree).all.all.find(_.isInstanceOf[OrigReturnType]) match {
              ...
              case _ =>
                throw new AssertionError("OrigReturnType attachment not found. (mitosNormalization adds it)")
            }
